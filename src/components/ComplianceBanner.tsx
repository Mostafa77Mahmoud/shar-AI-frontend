...
const ComplianceBanner: React.FC = () => {
  ...
  const { 
    ...
    currentUserEffectiveWarningCount,
  ...
  const getComplianceColorClasses = () => {
    if (overallCompliancePercentage >= 80) return {
      ...
      progressBg: "bg-shariah-green/20 dark:bg-green-900/40",
      progressFill: "bg-shariah-green dark:bg-green-500"
    };
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-4">
        ...
        <motion.div 
          ...
          className="bg-background/70 dark:bg-gray-800/40 p-3 rounded-lg shadow-sm text-center hover:shadow-md transition-shadow card-hover"
        >
          <div className="text-xl sm:text-2xl font-bold text-shariah-orange dark:text-amber-400">{currentUserEffectiveWarningCount}</div>
          <div className="text-muted-foreground text-xs sm:text-sm">{t('compliance.warningTerms')}</div>
        </motion.div>
        <motion.div 
          custom={2}
          variants={cardVariants}
      ...
      <div className="relative">
        <Progress 
          ...
          className={cn("h-3 sm:h-4 rounded-full transition-all duration-300", colors.progressBg)}
