...
const ContractTermsList: React.FC = () => {
  ...
  const handleManualRefresh = useCallback(async () => {
    ...
    toast({
      title: refreshed ? t('term.refreshed') || 'Session refreshed' : t('error.refreshFailed') || 'Refresh failed',
      variant: refreshed ? 'default' : 'destructive',
    });
  }, [refreshSessionData, toast, t]);
  ...
  const handleSaveAndReviewEditedSuggestion = useCallback(async (termId: string) => {
    ...
    addDecisionHistory({
      ...
      termId,
      details: 'User edited suggestion before AI review'
    });
  ...
  const filteredTerms = useMemo(() => {
    ...
    return analysisTerms.filter(term => {
      ...
      // Calculate effective status using same logic as compliance banner
      let effectiveStatus = term.compliance_status || 'compliant';
      
      if (term.expert_override_is_valid_sharia !== null && term.expert_override_is_valid_sharia !== undefined) {
        effectiveStatus = term.expert_override_is_valid_sharia ? 'compliant' : 'non_compliant';
      } else if (term.isUserConfirmed) {
        effectiveStatus = term.compliance_status || 'compliant';
      } else if (term.reviewedSuggestionStatus) {
        effectiveStatus = term.reviewedSuggestionStatus;
      } else if (term.isReviewedSuggestionValid !== null && term.isReviewedSuggestionValid !== undefined) {
        effectiveStatus = term.isReviewedSuggestionValid ? 'compliant' : 'non_compliant';
      }

      if (activeFilter === 'compliant') return effectiveStatus === 'compliant';
      if (activeFilter === 'warning') return effectiveStatus === 'warning';
      if (activeFilter === 'non-compliant') return effectiveStatus === 'non_compliant';
      return true;
    });
  ...
  const openExpertFeedbackForm = useCallback((term: FrontendAnalysisTerm) => {
    ...
    setCurrentExpertFeedback({
        ...
        expertComment: "",
        expertIsValidSharia: term.expert_override_is_valid_sharia ?? (term.compliance_status === 'compliant'),
  ...
  const submitExpertFeedback = useCallback(async () => {
    ...
    try {
        ...
        addDecisionHistory({
          ...
          actor: 'expert',
          timestamp: new Date().toISOString(),
          termId: expertFeedbackTermId,
          details: 'Expert feedback submitted'
        });
  ...
  return (
    <div className={`mt-8 space-y-6`} dir={dir} style={{ textAlign: dir === 'rtl' ? 'right' : 'left' }}>
      ...
      >
        ...
        {sessionId && Array.isArray(analysisTerms) && analysisTerms.length > 0 && (
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="ghost" onClick={handleManualRefresh} className="gap-2"
                disabled={isRefreshingSession || isFetchingSession}>
                {isRefreshingSession || isFetchingSession ? <Loader className="h-4 w-4 animate-spin" /> : <RefreshCw size={16} />}
                {t('term.refresh') || 'Refresh'}
              </Button>
            </div>
        )}
      </motion.div>
      ...
      {Array.isArray(decisionHistory) && (
        <DecisionHistoryPanel history={decisionHistory} className="mt-2" />
      )}

      <Tabs defaultValue="all" className="w-full" onValueChange={setActiveFilter}>
        <TabsList className="grid w-full grid-cols-4 mb-4 bg-muted dark:bg-gray-800">
          {['all', 'compliant', 'warning', 'non-compliant'].map(filterValue => (
            <TabsTrigger
                      ...
                      <Collapsible open={expandedTerms[term.term_id] || false} >
                        <CollapsibleTrigger onClick={() => toggleTerm(term.term_id)}
                          ...
                          <p
                            ...
                            style={textStyle}
                            dangerouslySetInnerHTML={{ __html: truncateText(term.term_text).replace(/(AAOIFI|IFSB|CIBAFI)/gi, '<span dir="ltr" style="unicode-bidi: embed; display: inline-block;">$1</span>') }}
                          />
                          <div className="flex flex-col items-end gap-2 shrink-0 ml-2 md:ml-4"> <span className={cn("term-tag px-3 py-1 text-xs font-bold rounded-full shadow-sm", effectiveStatus === 'compliant' ? 'bg-shariah-green text-white' : effectiveStatus === 'warning' ? 'bg-shariah-orange text-white' : 'bg-shariah-red text-white')}> {effectiveStatus === 'compliant' ? t('term.compliant') : effectiveStatus === 'warning' ? t('term.warning') : t('term.non-compliant')} </span> <motion.div animate={{ rotate: expandedTerms[term.term_id] ? 180 : 0 }} transition={{ duration: 0.2 }}> <ChevronDown size={20} className="text-muted-foreground" /> </motion.div> </div>
                        </CollapsibleTrigger>
                        <CollapsibleContent className="overflow-hidden">
                          <div className="p-4 md:p-5 border-t border-border bg-background/5 dark:bg-gray-800/10" style={textStyle}>
                                                        <div className="mb-4 p-3 bg-muted/50 dark:bg-gray-700/30 rounded-md border border-border/50 space-y-2">
                              <div className="text-xs space-y-1">
                                <div className="flex items-start gap-2"><span className="font-semibold text-muted-foreground min-w-[120px]">ID:</span><code className="text-foreground bg-background/50 px-2 py-0.5 rounded text-xs">{term.term_id}</code></div>
                                <div className="flex items-center gap-2"><span className="font-semibold text-muted-foreground min-w-[120px]">Status:</span><span className={cn("px-2 py-0.5 rounded text-xs font-semibold", effectiveStatus === 'compliant' ? 'bg-shariah-green/20 text-shariah-green' : effectiveStatus === 'warning' ? 'bg-shariah-orange/20 text-shariah-orange' : 'bg-shariah-red/20 text-shariah-red')}>{effectiveStatus === 'compliant' ? t('term.compliant') : effectiveStatus === 'warning' ? 'Warning' : t('term.non-compliant')}</span></div>
                              </div>
                            </div>
                            <div className="grid lg:grid-cols-2 gap-4 md:gap-5">
                              <div className="p-4 bg-card rounded-md shadow space-y-4 border border-border/50">
                                ...
                                {(effectiveStatus === 'non_compliant' || effectiveStatus === 'warning') && !term.isUserConfirmed && term.sharia_issue && !term.reviewedSuggestionIssue && ( <div className={`p-3.5 rounded-md shadow-inner border-l-4 ${effectiveStatus === 'warning' ? 'bg-amber-50 dark:bg-amber-900/30 border-amber-500 dark:border-amber-400' : 'bg-red-50 dark:bg-red-900/40 border-red-500 dark:border-red-400'}`}> <h4 className={`text-xs font-semibold mb-1 flex items-center ${effectiveStatus === 'warning' ? 'text-amber-700 dark:text-amber-300' : 'text-red-700 dark:text-red-300'}`}> <AlertCircle size={15} className={dir === 'rtl' ? "ml-1.5" : "mr-1.5"} /> {t('term.why')} </h4> <p className={cn(`text-sm whitespace-pre-wrap leading-relaxed ${effectiveStatus === 'warning' ? 'text-amber-800 dark:text-amber-200' : 'text-red-800 dark:text-red-200'}`)} style={textStyle} dangerouslySetInnerHTML={{ __html: (term.sharia_issue || "").replace(/(AAOIFI|IFSB|CIBAFI)/gi, '<span dir="ltr" style="unicode-bidi: embed; display: inline-block;">$1</span>') }} /> </div> )}
                                {term.reviewedSuggestionIssue && effectiveStatus === 'non_compliant' && ( <div className="p-3.5 rounded-md bg-red-50 dark:bg-red-900/40 border-l-4 border-red-500 dark:border-red-400 shadow-inner mt-2"> <h4 className="text-xs font-semibold text-red-700 dark:text-red-300 mb-1 flex items-center"> <AlertCircle size={15} className={dir === 'rtl' ? "ml-1.5" : "mr-1.5"}/> {t('term.newShariaIssue')} </h4> <p className={cn("text-sm text-red-800 dark:text-red-200 whitespace-pre-wrap leading-relaxed")} style={textStyle} dangerouslySetInnerHTML={{ __html: (term.reviewedSuggestionIssue || "").replace(/(AAOIFI|IFSB|CIBAFI)/gi, '<span dir="ltr" style="unicode-bidi: embed; display: inline-block;">$1</span>') }}/> </div> )}
                                {term.reference_number && ( <div className={`p-3.5 rounded-md shadow-inner border-l-4 ${ (effectiveStatus === 'compliant') ? 'bg-blue-50 dark:bg-blue-900/40 border-blue-500 dark:border-blue-400' : 'bg-yellow-50 dark:bg-yellow-900/30 border-yellow-500 dark:border-yellow-400'}`}> <h4 className={`text-xs font-semibold mb-1 flex items-center ${ (effectiveStatus === 'compliant') ? 'text-blue-700 dark:text-blue-300' : 'text-yellow-700 dark:text-yellow-300'}`}> <Info size={15} className={dir === 'rtl' ? "ml-1.5" : "mr-1.5"} /> {t('term.reference')} </h4> <p className={cn(`text-sm whitespace-pre-wrap leading-relaxed max-w-md ${ (effectiveStatus === 'compliant') ? 'text-blue-800 dark:text-blue-200' : 'text-yellow-800 dark:text-yellow-200'}`)} style={textStyle} dangerouslySetInnerHTML={{ __html: term.reference_number.replace(/(AAOIFI|IFSB|CIBAFI)/gi, '<span dir="ltr" style="unicode-bidi: embed; display: inline-block;">$1</span>') }}/> </div> )}
                              </div>
                              <div className="p-4 bg-card rounded-md shadow space-y-4 border border-border/50">
                                ...
                                : textInSuggestionOrEditBox && (effectiveStatus !== 'compliant' || term.userModifiedText || term.reviewedSuggestion ) ? ( <div className="space-y-2"> <h4 className="text-xs font-bold text-muted-foreground mb-1.5 uppercase tracking-wider"> {t(suggestionBoxLabelKey)} </h4> <div className="p-3.5 text-base rounded-md bg-blue-50 dark:bg-blue-900/40 border border-blue-300 dark:border-blue-700/60 text-blue-800 dark:text-blue-200 whitespace-pre-wrap leading-relaxed shadow-inner"> <p className={cn(dir === 'ar' && "text-right")} style={textStyle} dangerouslySetInnerHTML={{ __html: textInSuggestionOrEditBox.replace(/(AAOIFI|IFSB|CIBAFI)/gi, '<span dir="ltr" style="unicode-bidi: embed; display: inline-block;">$1</span>') }} /> </div> {term.reviewedSuggestion && (term.userModifiedText === term.reviewedSuggestion || textInSuggestionOrEditBox === term.reviewedSuggestion) && ( <div className={`text-xs mt-1 p-1 rounded ${term.reviewedSuggestionStatus === 'compliant' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : term.reviewedSuggestionStatus === 'warning' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'}`}> AI Review: {term.reviewedSuggestionStatus === 'compliant' ? t('review.looksGood') : `${t('review.concern')}: ${term.reviewedSuggestionIssue || t('review.complianceIssue')}`} </div> )} <div className="flex flex-wrap gap-2 pt-2"> <Button size="sm" onClick={() => handleConfirmChanges(term)} className="bg-shariah-green hover:bg-shariah-green/90 flex-grow sm:flex-grow-0 press-effect" disabled={(isTermProcessing && isTermProcessing[term.term_id]) || (isReviewingModification && isReviewingModification[term.term_id])}> {(isTermProcessing && isTermProcessing[term.term_id]) ? <Loader className="h-4 w-4 animate-spin"/> : <ThumbsUp size={16}/>} {(isTermProcessing && isTermProcessing[term.term_id]) ? t('processing') : t('button.confirm')} </Button> <Button variant="outline" size="sm" onClick={() => handleEditSuggestion(term)} className="flex-grow sm:flex-grow-0 press-effect border-input hover:bg-accent" disabled={(isTermProcessing && isTermProcessing[term.term_id]) || (isReviewingModification && isReviewingModification[term.term_id])}><Edit3 size={16}/> {t('term.editSuggestion')}</Button> </div> </div> )
                                : ( effectiveStatus === 'compliant' && !term.isUserConfirmed && !term.reviewedSuggestion && ( <div className="p-3.5 text-base rounded-md bg-green-50 dark:bg-green-900/30 border border-green-300 dark:border-green-700/50 text-green-700 dark:text-green-300" style={textStyle}> <p>{t('term.alreadyCompliant')}</p> </div> )
                                )}
                                {term.currentQaAnswer && ( <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-3 pt-3 border-t border-border"> <h4 className="text-xs font-bold text-muted-foreground mb-1.5 uppercase tracking-wider flex items-center gap-1.5"> <MessageSquare size={14} className="text-blue-500" /> {t('term.answer')} </h4> <div className={cn("mt-1 p-3.5 text-base rounded-md bg-blue-50 dark:bg-blue-900/40 border border-blue-200 dark:border-blue-500/40 text-foreground whitespace-pre-wrap leading-relaxed shadow-inner")} style={textStyle}> {term.currentQaAnswer} </div> {!term.isUserConfirmed && effectiveStatus !== 'compliant' && ( <Button variant="link" size="sm" className="text-blue-600 dark:text-blue-400 px-0 h-auto py-1 mt-1.5 text-xs" onClick={() => handleUseAnswerAsSuggestion(term)} disabled={(isTermProcessing && isTermProcessing[term.term_id]) || (isReviewingModification && isReviewingModification[term.term_id])}> {((isReviewingModification && isReviewingModification[term.term_id]) && term.currentQaAnswer === textInSuggestionOrEditBox) ? <Loader className="h-3 w-3 animate-spin"/> : <Sparkles size={14}/>} {t('button.useAndReview')} </Button> )} </motion.div> )}
